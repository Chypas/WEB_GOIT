...
class AddressBook(UserDict, AbstractOut):
    ...
    def show_contacts_by_birthday(self, days):
        result = [self.data[record] for record in self.data if
                  self.data[record].birthday.value != None and self.data[record].days_to_birthday()
                  <= days]
        return result
...
class Record:
    ...
    def __str__(self):
        return f"Name: {self.name.value} Phone: {self.phones} Address: {self.address.value if self.address else None}" \
               f" Email: {self.email.value if self.email else None} Birthday: {self.birthday.value if self.birthday else None}"
...
# Creating user's notebooks
class NoteBook(UserDict, AbstractOut):
    def __init__(self):
        self.data = {}
